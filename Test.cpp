...
TEST_SUITE(" Point moveTowards function") {
    ...
    TEST_CASE("TEST 8: teams members checking")
    {
        ...
        Team t_1(n1);
        Cowboy *c1 =new Cowboy("Hikari", Point(12, 16));
        Cowboy *c2 =new Cowboy("hi", Point(31, 12));
        t_1.add(c1);
        t_1.add(c2);
        Team t_2(n2);
        ...
        while (t_1.stillAlive() > 0 && t_2.stillAlive() > 0) {
            t_1.attack(&t_2);
            t_2.attack(&t_1);
            if(t_1.stillAlive()==0 || t_2.stillAlive()==0)break;
        }
...
TEST_SUITE(" Ninja slash function") {
    ...
    TEST_CASE("Test 14: Cowboy Reload Method") {
        Cowboy *c1 = new Cowboy("Jake", Point(3,3));
        Character *c2= new Character(100, "John", Point(4,4));
        for (int i = 0; i < 6; i++) {
            c1->shoot(c2);
        }
        CHECK(!c1->hasboolets());
...
TEST_CASE("Test 20: Team Constructor and add method") {
    Cowboy *cap = new Cowboy( "John", Point(10,10));
    Team *team = new Team(cap);
    CHECK_NOTHROW(Team *temp = new Team(cap));
    Cowboy *player = new Cowboy("Jane", Point(5,5));
    team->add(player);
...
TEST_CASE("Test 22: Print checks"){
    Character c (100,"melkor", Point(5,10));
    string s = "";
    s.append("name:"+c.getName());
    s.append("\nHP:100");
    s.append("\nPosition:");
    s.append(c.getLocation().toString());
    string act = c.print();
    CHECK_EQ(s,act);

}
TEST_CASE("Test 23: ninja hit check"){

        //valid range
    Character *c = new Character(100,"melkor", Point(5,10));
    YoungNinja *legolas =new YoungNinja("maneve",Point(0,10));
    legolas -> slash(c);
    CHECK_EQ(c->getHP(),60);

    // unvalid range
    Character *c1 = new Character(100,"eru", Point(5,20));
    YoungNinja *gimli =new YoungNinja("maneve",Point(0,0));
    gimli -> slash(c1);
